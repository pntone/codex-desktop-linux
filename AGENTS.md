...
- `scripts/lib/bundled-plugins.sh`
  Browser Use, Chrome, and Linux Computer Use plugin staging; Chrome native-host injection; Linux Computer Use backend build; COSMIC helper build; and bundled-plugin marketplace generation.
- `scripts/lib/patch-chrome-plugin.js`
  Linux compatibility patcher for the upstream bundled Chrome plugin scripts. Adds Linux manifest checks, Chrome/Brave/Chromium native-host manifest coverage, Linux browser profile fallback, and Brave/Chromium-aware diagnostics when upstream has not already shipped them.
...
- `computer-use-linux/`
  Rust crate implementing Linux native plugin binaries: the Linux Computer Use MCP backend (`codex-computer-use-linux`), the Chrome native-messaging host (`codex-chrome-extension-host`), and helper binaries. Computer Use talks AT-SPI to read accessibility trees, captures screenshots through GNOME Shell DBus or XDG Desktop Portal, lists/focuses windows through `src/windowing/` backends, and synthesizes input via `ydotool`.
- `computer-use-linux/src/bin/codex-chrome-extension-host.rs`
  Linux native-messaging host for the bundled Chrome plugin. Bridges Chrome extension stdio frames to local Browser Use Unix-socket clients, validates the socket directory and same-UID peer credentials, watches Codex session rollout files for completed turns, emits `turnEnded`, and is staged as `extension-host/linux/<arch>/extension-host`.
